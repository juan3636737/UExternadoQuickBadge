...
import pandas as pd
from app.models import Gestión_información, Nombreestrategia, Codigo, NivelRutaDocente

class Command(BaseCommand):
    help = 'Carga datos para el modelo Gestión_información desde un archivo Excel.'
        ...
        try:
            ...
            for index, row in df.iterrows():
                try:
                    ...
                    # Crear registro en Gestión_información
                    try:
                        Gestión_información.objects.create(
                        ...
                        self.stdout.write(self.style.SUCCESS(f'Datos cargados para Gestión_información con índice {index}'))
                    except Exception as e:
                        self.stdout.write(self.style.ERROR(f'Error al crear Gestión_información en el índice {index}: {str(e)}'))
                        continue
