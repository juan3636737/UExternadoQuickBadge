...
import pandas as pd
from app.models import Producción_de_recursos, Nombreestrategia, Codigo, NivelRutaDocente

class Command(BaseCommand):
    help = 'Carga datos para el modelo Producción_de_recursos desde un archivo Excel.'
        ...
        try:
            ...
            for index, row in df.iterrows():
                try:
                    ...
                    # Verifica si el nombre_estrategia_capacitacion se ha creado con el código proporcionado
                    if nombre_estrategia_capacitacion.codigo != codigo:
                        self.stdout.write(self.style.ERROR(f'El nombre de estrategia "{row.get("nombre_estrategia_capacitacion", "")}" no se ha creado con el código proporcionado.'))
                        continue
                    ...
                    # Crear registro en Producción_de_recursos
                    Producción_de_recursos.objects.create(
                    ...
                    self.stdout.write(self.style.SUCCESS(f'Datos cargados para Producción_de_recursos con índice {index}'))
